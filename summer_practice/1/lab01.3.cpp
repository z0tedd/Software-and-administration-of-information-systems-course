...
int main() {
  ...
  size_t avgHeight = accumulate(heights.begin(), heights.end(), 0) /
  ...
  for (int i = 0; i < bin_count; i++) {
    ...
    if (heights[i] > avgHeight) {
      ...
      for (int j = avgHeight; j < heights[i]; j++) {
        cout << '+';
      }
    }
    if (heights[i] <= avgHeight) {
      ...
      for (int j = heights[i]; j < avgHeight; j++) {
        cout << '_';
      }
    }
    cout << '\n';
  }
  // cerr << height << ' ' << (static_cast<double>(count) / max_count) << '\n';
  return 0;
}
